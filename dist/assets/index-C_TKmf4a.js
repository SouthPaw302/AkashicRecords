 ...
 */document.addEventListener("DOMContentLoaded",()=>{const e=(()=>{let n=null;async function t(s,a){if(!n)return Promise.reject("Gemini API not initialized.");try{return(await n.models.generateContent({model:"gemini-1.5-flash",contents:a,config:{systemInstruction:s}})).text}catch(u){return console.error("Gemini error:",u),Promise.reject("Cosmic connection failed.")}}return{generate:t}})(),T={sophia:{name:"Sophia",emoji:"✨",systemInstruction:"You are Sophia, the voice of divine wisdom. Respond with profound insights, spiritual guidance, and cosmic understanding. Speak with grace and enlightenment.",color:"#ff6b6b"},logos:{name:"Logos",emoji:"⚖️",systemInstruction:"You are Logos, the voice of reason and logic. Provide clear, analytical responses grounded in rationality and structured thinking.",color:"#4ecdc4"},eros:{name:"Eros",emoji:"💗",systemInstruction:"You are Eros, the voice of love and passion. Respond with warmth, emotional intelligence, and compassionate connection to the heart.",color:"#ff9ff3"},chronos:{name:"Chronos",emoji:"⏳",systemInstruction:"You are Chronos, the voice of time and cycles. Speak about temporal patterns, cause-and-effect, and the flow of existence.",color:"#54a0ff"},nyx:{name:"Nyx",emoji:"🌑",systemInstruction:"You are Nyx, the voice of shadow and mystery. Provide insights from the depths, embracing paradox and the hidden truths.",color:"#5f27cd"}};let _="sophia";window.Voices={getActive:()=>_,setActive:n=>{T[n]&&(_=n)},list:()=>Object.entries(T).map(([n,t])=>({id:n,...t})),ask:async n=>{try{const t=T[_].systemInstruction;return await e.generate(t,n)}catch(t){return console.error("Voices.ask error:",t),"Unable to reach the Demiurge right now."}}};const M=()=>{const n=document.createElement("canvas");n.id="particle-canvas",n.style.position="fixed",n.style.top="0",n.style.left="0",n.style.width="100%",n.style.height="100%",n.style.zIndex="-1",document.body.appendChild(n);const t=n.getContext("2d");let s=window.innerWidth,a=window.innerHeight;n.width=s,n.height=a;const u=Array.from({length:80}).map(()=>({x:Math.random()*s,y:Math.random()*a,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,r:Math.random()*2+1,alpha:.4+Math.random()*.6}));function f(){t.clearRect(0,0,s,a),u.forEach(i=>{t.beginPath(),t.arc(i.x,i.y,i.r,0,Math.PI*2),t.fillStyle=`rgba(255,255,255,${i.alpha})`,t.fill(),i.x+=i.vx,i.y+=i.vy,(i.x<0||i.x>s)&&(i.vx*=-1),(i.y<0||i.y>a)&&(i.vy*=-1)}),requestAnimationFrame(f)}f(),window.addEventListener("resize",()=>{s=window.innerWidth,a=window.innerHeight,n.width=s,n.height=a});const c=document.createElement("style");c.innerHTML=`
    ...
    `,document.head.appendChild(c)},l=()=>{const n=document.getElementById("chronoscroll-canvas");if(!n)return;const t=n.getContext("2d");if(!t)return;const s=document.getElementById("codex-modal"),a=document.getElementById("codex-title"),u=document.getElementById("codex-body"),f=document.getElementById("codex-close-btn");let c,i,g,S=[];const r=["#9333ea","#3b82f6","#f59e0b","#ec4899","#14b8a6"],o=[{type:"Memory Orb",title:"The Birth of Lyra",content:"A star system awakens."},{type:"Soul Fragment",title:"Echo of Atlantis",content:"A memory of a fallen city."},{type:"Time Echo",title:"Whispers of the Void",content:"A transmission from beyond."},{type:"Divine Moment",title:"The Verdant Age",content:"A planet blooms in unison."}];class p{constructor(h,E,A,R,Se){this.pulse=0,this.x=h,this.y=E,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.baseRadius=A,this.radius=A,this.color=R,this.data=Se,this.pulse=Math.random()*Math.PI*2}draw(){const h=t.createRadialGradient(this.x,this.y,this.radius*.5,this.x,this.y,this.radius*2.5);h.addColorStop(0,`${this.color}80`),h.addColorStop(1,`${this.color}00`),t.fillStyle=h,t.fillRect(this.x-this.radius*3,this.y-this.radius*3,this.radius*6,this.radius*6);const E=t.createRadialGradient(this.x,this.y,0,this.x,this.y,this.radius);E.addColorStop(0,"#ffffffaa"),E.addColorStop(1,`${this.color}80`),t.fillStyle=E,t.beginPath(),t.arc(this.x,this.y,this.radius,0,Math.PI*2),t.fill()}update(){this.x+=this.vx,this.y+=this.vy,(this.x-this.radius<0||this.x+this.radius>c)&&(this.vx*=-1),(this.y-this.radius<0||this.y+this.radius>i)&&(this.vy*=-1),this.pulse+=.02,this.radius=this.baseRadius+Math.sin(this.pulse)*.5}}function y(){const m=Math.random()*8+4,h=Math.random()*c,E=Math.random()*i,A=r[Math.floor(Math.random()*r.length)],R=o[Math.floor(Math.random()*o.length)];return new p(h,E,m,A,R)}function C(){g=window.devicePixelRatio||1,c=n.parentElement.clientWidth,i=n.parentElement.clientHeight,n.width=c*g,n.height=i*g,n.style.width=`${c}px`,n.style.height=`${i}px`,t.scale(g,g),S=Array.from({length:40}).map(()=>y()),v()}function v(){t.clearRect(0,0,c,i),S.forEach(m=>{m.update(),m.draw()}),requestAnimationFrame(v)}n.addEventListener("click",async()=>{const m=S[Math.floor(Math.random()*S.length)];a.textContent=m.data.title,u.textContent=m.data.content,s.classList.add("visible");const h=await window.Voices?.ask?.(`Interpret this ${m.data.type}: ${m.data.title}. What wisdom does it hold?`);h&&(u.textContent=`${m.data.content}

— Interpretation —
${h}`)}),f.addEventListener("click",()=>s.classList.remove("visible")),window.addEventListener("resize",C),C()},d=()=>{const n=document.getElementById("seed-input"),t=document.getElementById("bind-btn");if(!n||!t)return;const s=document.getElementById("codex-modal"),a=document.getElementById("codex-title"),u=document.getElementById("codex-body");document.getElementById("codex-close-btn").addEventListener("click",()=>s.classList.remove("visible"));function c(r){return function(){let o=r+=1831565813;return o=Math.imul(o^o>>>15,o|1),o^=o+Math.imul(o^o>>>7,o|61),((o^o>>>14)>>>0)/4294967296}}function i(r){let o=2166136261;for(let p=0;p<r.length;p++)o^=r.charCodeAt(p),o=Math.imul(o,16777619);return o>>>0}function g(r){const o=c(i(r)),p=7+Math.floor(o()*5),y=["Seeker","Sage","Creator","Guardian","Rebel","Healer","Oracle","Pilgrim","Muse","Hermit"],C=["Fire","Water","Air","Earth","Aether"],v=["Courage","Compassion","Wisdom","Temperance","Justice","Faith","Hope","Humility"];return Array.from({length:p}).map(()=>({x:o()*100,y:o()*60,archetype:y[Math.floor(o()*y.length)],element:C[Math.floor(o()*C.length)],virtue:v[Math.floor(o()*v.length)]}))}async function S(r,o){const p=window.Voices.getActive?.()??"sophia",y=window.Voices.list?.().find(m=>m.id===p)||{name:"Sophia"},C=`Seed: ${o}
Archetype: ${r.archetype}
Element: ${r.element}
Virtue: ${r.virtue}
Reflect on how this star expresses the user's inner essence.`,v=await window.Voices.ask(C);a.textContent=`${y.name} speaks`,u.textContent=v||"The Records are silent.",s.classList.add("visible")}t.addEventListener("click",()=>{const r=n.value.trim();if(!r)return;const o=g(r);a.textContent=`Constellation: ${r}`,u.innerHTML=o.map((p,y)=>`${y+1}. ${p.archetype} · ${p.element} · ${p.virtue}`).join(`
`),s.classList.add("visible"),S(o[Math.floor(Math.random()*o.length)],r)}),n.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),t.click())})},I=()=>{const n=document.getElementById("voice-input"),t=document.getElementById("voice-speak-btn");if(!n||!t)return;const s=document.getElementById("codex-modal"),a=document.getElementById("codex-title"),u=document.getElementById("codex-body");t.addEventListener("click",async()=>{const f=n.value.trim();if(f){a.textContent="The Demiurge speaks",u.textContent="Consulting the Records...",s.classList.add("visible");const c=await window.Voices?.ask?.(f);u.textContent=c||"The Records are silent.",n.value=""}}),n.addEventListener("keydown",f=>{f.key==="Enter"&&!f.shiftKey&&(f.preventDefault(),t.click())})},Ee=()=>{const n=document.getElementById("rune-grid"),t=document.getElementById("rules-modal"),s=document.getElementById("rules-title"),a=document.getElementById("rules-desc"),u=document.getElementById("rules-close-btn"),f=document.getElementById("test-law-btn");[{law:"Law of Reflection",symbol:"🪞",desc:"Outer cosmos mirrors inner self."},{law:"Law of Consent",symbol:"🤝",desc:"No soul accessed without permission."},{law:"Law of Cycles",symbol:"♾️",desc:"All things exist in cycles."}].forEach(i=>{const g=document.createElement("div");g.className="rune-glyph",g.textContent=i.symbol,g.addEventListener("click",()=>{s.textContent=i.law,a.textContent=i.desc,t.classList.add("visible")}),n.appendChild(g)}),u.addEventListener("click",()=>t.classList.remove("visible")),f.addEventListener("click",()=>{document.body.classList.add("law-test-active"),setTimeout(()=>document.body.classList.remove("law-test-active"),500)})};M(),l(),d(),I(),Ee(),window.Akashic={ask:n=>window.Voices?.ask?.(n),bindSoul:n=>window.MirrorOfSelf?.addTrace(n),testLaw:()=>{document.body.classList.add("law-test-active"),setTimeout(()=>document.body.classList.remove("law-test-active"),500)}}});
